...
const AiModule = isTurboModuleEnabled ? require('./NativeAi').default : NativeModules.Ai;
...
export function getModel(modelId, options = {}) {
  return new AiModel(modelId, options);
}
export async function getModels() {
  return Ai.getModels();
}
export async function downloadModel(modelId, callbacks) {
  const eventEmitter = new NativeEventEmitter(NativeModules.Ai);
...
export async function prepareModel(modelId) {
